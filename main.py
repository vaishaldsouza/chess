...
def get_unique_filename(base_path):
    """Generate a unique filename to prevent FileExistsError when archiving games"""
...
def parse_issue(title):
    ...
    if title.strip().lower() == 'chess: start new game':
...
def main(issue, issue_author, repo_owner):
    ...
    if action[0] == Action.NEW_GAME:
        ...
        game.headers['Result'] = gameboard.result(claim_draw=True)

    elif action[0] == Action.RESIGN:
        if not os.path.exists('games/current.pgn'):
            return False, 'ERROR: There is no game in progress! Start a new game first'
            ...
            with open('data/last_moves.txt') as moves_file:
                ...
                for line in lines:
                    if ':' in line:
                        ...
                        if last_player is None and 'Start game' not in parts[0]:
                            last_player = player

        is_owner = (issue_author == repo_owner)
        is_participant = (issue_author in player_list)
        is_turn = (issue_author != last_player)

        if not is_owner and (not is_participant or not is_turn):
            reason = "Only players who have participated in the current game and whose turn it is can resign."
            issue.create_comment(settings['comments']['invalid_resign'].format(author=issue_author, reason=reason))
            issue.edit(state='closed', labels=['Invalid'])
            return False, 'ERROR: Invalid resignation request'

        is_resignation = True
        result = '0-1' if gameboard.turn == chess.WHITE else '1-0'
    ...
    if gameboard.is_game_over(claim_draw=True) or is_resignation:
        win_msg = {
            '1/2-1/2': 'It\'s a draw',
            '1-0': 'White wins',
            '0-1': 'Black wins'
        ...
        result_str = game.headers['Result'] if is_resignation else gameboard.result(claim_draw=True)
        ...
        outcome_text = win_msg.get(result_str, 'UNKNOWN')
        if is_resignation:
            outcome_text += " (resignation)"
        ...
        target_filename = get_unique_filename(datetime.now().strftime('games/game-%Y%m%d-%H%M%S.pgn'))
        os.rename('games/current.pgn', target_filename)
