...
def get_test_data(settings, move_data, owner, is_white_turn):
    labels = []
    comments = []
    
    if 'is_invalid' in move_data and move_data['is_invalid'] == True:
        labels += ['Invalid']
        if 'Resign' in move_data['move']:
...
def run_test_case(filename, main_fn):
    ...
    for i in range(len(test_data['moves'])):
        ...
        labels, comments = get_test_data(settings, move_data, test_data['owner'], is_white_turn)
